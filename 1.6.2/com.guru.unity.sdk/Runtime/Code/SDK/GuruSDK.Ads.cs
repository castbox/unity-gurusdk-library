...
namespace Guru
{
    ...
    public partial class GuruSDK
    {
        ...
        private static readonly bool _adInvokeEventOnMainThread = false;
            ...
            try
            {
                ...
                switch (mediationProfile.mediationType)
                {
                    ...
                    default:
                        ...
                        try
                        {
                            if (Instance._initConfig != null && Instance._initConfig.AutoSetHasUserConsent)
                            {
                                if (!IsEea())
                                {
                                    Guru.Ads.Max.MaxAdManager.SetHasUserConsent(true);
                                }
                            }
                        }
                        catch (Exception ex)
                        ...
                        adManager = new Guru.Ads.Max.MaxAdManager(AdService.Instance, 
                            mediationProfile, 
                            OnAdsInitComplete, 
                            _adInvokeEventOnMainThread); // 这里需要关闭 MAX 的主线程回归操作
                        break;
                }
                
                // 启动广告服务
                AdService.Instance.StartService(adManager, BlockedAdEvents); // 传入广告管理器以及事件过滤列表， 主线程监控不用传值，靠自动
            }
            catch (Exception e)
            {
