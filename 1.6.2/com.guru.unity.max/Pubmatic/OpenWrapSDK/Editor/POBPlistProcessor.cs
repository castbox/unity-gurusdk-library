...
public class POBPlistProcessor
{
    ...
    private static List<string> ReadSKAdNetworkIDsFile()
    {
        ...
        // string path = Path.Combine($"{Application.dataPath}/../Packages/com.guru.unity.max/Pubmatic", SKADNETWORKS_FILE_PATH);
        var dir = GetDirPath();
        if (string.IsNullOrEmpty(dir) || !Directory.Exists(dir))
        {
            throw new Exception($"Dir not found for {nameof(POBPlistProcessor)}, skAdNetworkIds will noty add to info.plist");
        }
        // var path = Path.Combine(dir, SKADNETWORKS_FILE_PATH);
        var path = Path.Combine(dir, "OpenWrapSDKSKAdNetworkIDs.xml");
        try
        {
            if (!File.Exists(path))
            {
...
#pragma warning restore 0168
    ...
    {
        var guids = AssetDatabase.FindAssets($"{nameof(POBPlistProcessor)} t:Script");
