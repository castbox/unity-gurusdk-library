namespace Guru.Editor
{
    ...
    public partial class AppBuilder
    {
	    ...
	    private const string DefaultEmbeddedPathValue = "default";
	    ...
	    {
		    if (string.IsNullOrEmpty(gradlePath)) return; // 使用现有配置路径

		    if (gradlePath == DefaultEmbeddedPathValue)
		    {
			    UsePlayerEmbeddedGradlePath(); // 使用默认配置
			    return;
		    }
	    ...
	    {
		    if (string.IsNullOrEmpty(jdkRoot)) return; // 使用现有配置路径
		    
		    if (jdkRoot == DefaultEmbeddedPathValue)
		    {
			    UsePlayerEmbeddedJDKPath(); // 使用默认配置
			    return;
		    }
	    ...
	    {
		    if (string.IsNullOrEmpty(ndkRoot)) return; // 使用现有配置路径
		    
		    if (ndkRoot == DefaultEmbeddedPathValue)
		    {
			    UsePlayerEmbeddedNDKPath(); // 使用默认配置
			    return;
		    }
	    ...
	    /// <summary>
	    /// 设置自定义的 AndroidSDK 路径
	    /// </summary>
	    /// <param name="sdkRoot"></param>
	    private static void SetAndroidSDKRoot(string sdkRoot)
	    {
		    if (string.IsNullOrEmpty(sdkRoot)) return; // 使用现有配置路径
		    
		    if (sdkRoot == DefaultEmbeddedPathValue)
		    {
			    UsePlayerEmbeddedAndroidSDKPath(); // 使用默认配置
			    return;
		    }
