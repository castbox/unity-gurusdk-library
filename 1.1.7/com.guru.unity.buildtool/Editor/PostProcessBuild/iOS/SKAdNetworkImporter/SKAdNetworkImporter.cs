...
namespace Guru.Editor
{
	...
	public static class SKAdNetworkImporter
	{
		...
		{
			var guids = AssetDatabase.FindAssets($"{nameof(SKAdNetworkImporter)}");
			if (guids.Length > 0)
			{
				var path = Directory.GetParent(AssetDatabase.GUIDToAssetPath(guids[0]))?.FullName ?? "";
				return path;
			}
			return $"{Application.dataPath.Replace("Assets", "Packages")}/{PACKAGE_NAME}/Editor/PostProcessBuild/iOS/SKAdNetworkImporter/SKAdNetworkImporter.cs";
		}
